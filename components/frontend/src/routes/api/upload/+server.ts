...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (!filename || typeof filename !== "string") {
      ...
      return json(
        { error: "Invalid request body: filename is required." },
        { status: 400 },
      )
    }
    ...
    logger.info(
      `Fetching S3 POST credentials from ${backendApiUrl} for filename: ${filename}`,
    )
    ...
    logger.info(
      ...
      "Successfully fetched S3 POST credentials.",
    )

...
export function onMediaTimeUpdate(
  ...
  const displaySubtitleEnIndex = getDisplaySubtitleIndex(
    currentTime,
    subtitles_en,
  )
...
function getDisplaySegmentNr(currentTime: number, segments: Segment[]) {
...
function getDisplaySubtitleIndex(currentTime: number, subtitles: Subtitle[]) {
  let currentSubtitle = subtitles.find(
...
function getDisplaySpeakerId(displaySegmentNr: number, segments: Segment[]) {
  if (displaySegmentNr) {

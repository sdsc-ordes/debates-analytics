...
- `odtp-unog-digitalrecordings-scrapper`. Component to scrap and download
  metadata from the UNOG Digital Recordings platform.
...
- `debates-dataloader`. Tool to upload files to the S3 bucket, MongoDB, and
  Solr.
...
Alternatively, you can add the components to the GUI and manually run the
execution.

### How to use this tool with a custom file?

In order to run the `out-py annotate-whisper` module, it is necessary to start
with a valid metadata JSON file. You can obtain one by fetching the data from
the scrapper or create a synthetic one manually. This is the example of a
synthetic one you can use to generate yours. It should validate against
...
This metadata should be placed in the `odtp-input` folder for the
`odtp-pyannote-whisper` component. In case you are using the outcome of the
scrapper you shouldn't manually create this initial metadata file.

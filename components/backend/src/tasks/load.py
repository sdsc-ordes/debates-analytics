...
def load_transcripts(media_id: str, s3_key: str):
    ...
    try:
        ...
        srt_keys = {
            "translation": f"{media_id}/transcripts/subtitles-translation.srt",
            "original": f"{media_id}/transcripts/subtitles-original.srt"
        }

        for k, v in srt_keys.items()
            logger.info(f"Downloading SRT: {v}")
            srt_content = s3.download_as_string(v)
            if not srt_content:
